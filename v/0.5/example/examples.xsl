...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
  ...
  <xsl:template name="head-example">
	...
	<xsl:text>sgvizler.option.namespace.</xsl:text><xsl:value-of select="prefix"/><xsl:text> = '</xsl:text><xsl:value-of select="url"/><xsl:text>';
      </xsl:text>
      </xsl:for-each>
      <xsl:text>
  ...
  <xsl:template name="examplesList">
	...
	<xsl:for-each select="/root/examples/example[endpoint/@ref = $endpointId]">
	  <li>
	    <a href="ex{$endpointName}{position()}.html" target="main">
  ...
  <xsl:template name="_sgvizlerDiv">
    ...
    <xsl:param name="chart">gTable</xsl:param>
